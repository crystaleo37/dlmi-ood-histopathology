...
Training a linear head takes $\sim$20\,s on cached features, so we train $N{=}5$ heads with different seeds and average sigmoid outputs~\citep{wortsman2022model}:
$\hat{p}(\mathbf{x}) = \frac{1}{NK} \sum_{i=1}^{N} \sum_{j=1}^{K} h_i(\text{backbone}(v_j(\mathbf{x})))$
...
Switching from DINOv2 to Phikon gives $+8.62$~pp, dwarfing all head-level interventions.
...
FroFA~\citep{bar2024frofa} gains diminish with data scale ($+3.9$\% few-shot, $\sim$$+0.1$\% at $\sim$100K samples), consistent with our results.

\textbf{Limitations and future work.}
(i)~UNI and Virchow2 outperform Phikon on multi-task benchmarks~\citep{nature2025benchmark}; a multi-backbone ensemble would likely improve further.
(ii)~LP-FT~\citep{kumar2022finetuning} could unlock backbone adaptation while preserving OOD robustness.
(iii)~Test-time adaptation via entropy minimisation (TENT,~\citet{wang2021tent}) could be applied to the backbone.
